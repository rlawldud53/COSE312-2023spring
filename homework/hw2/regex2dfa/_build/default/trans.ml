...
=fun nfa_t -> 
  (*let eclose_states = BatSet.empty in*)
    ...
    in let f_elem_num3 = BatSet.cardinal ( BatSet.filter (fun x -> BatSet.mem x (d0_closure) ) (Nfa.get_final_states (nfa_t) ) ) in 
        if f_elem_num3 >= 1 then Dfa.add_final_state dfa_t d0_closure
        else res
...
= fun dfa_t symbol_lst -> 
  ...
  = fun (dfa_t) (start) symbol_lst2 -> 
    ...
    | [] -> 
      ...
      (*else if start = Dfa.get_initial_state dfa_t then true*)
      else false
    | hd::tl ->
